...
data.file.epu = sapply(data.file.names,function(x) strsplit(x,paste0(data.prefix,'|_|.nc'))[[1]][2],USE.NAMES = F)
data.file.temp = sapply(data.file.names,function(x) strsplit(x,paste0(data.prefix,'|_|.nc'))[[1]][3],USE.NAMES = F)
data.file.z = sapply(data.file.names,function(x) strsplit(x,paste0(data.prefix,'|_|.nc'))[[1]][4],USE.NAMES = F)

area.file.names = list.files(area.dir)
area.file.epu = sapply(area.file.names,function(x) strsplit(x,paste0(area.prefix,'|_|.nc'))[[1]][2],USE.NAMES = F)
area.file.temp = sapply(area.file.names,function(x) strsplit(x,paste0(area.prefix,'|_|.nc'))[[1]][3],USE.NAMES = F)
area.file.z = sapply(area.file.names,function(x) strsplit(x,paste0(area.prefix,'|_|.nc'))[[1]][4],USE.NAMES = F)
...
for(i in 1:length(data.file.names)){
  ...
  area.daily.out = paste0('daily_area_',data.file.epu[i],'_',data.file.temp[i],'_',data.file.z[i],'.rds')
