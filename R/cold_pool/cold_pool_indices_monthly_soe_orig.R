...
for (y in sort(unique(dt_bt_sep_oct$year))) { # for each year
  ...
  dt_cell_cold <- dt_pre_year %>% group_by(cell_no) %>%
    summarise(length_below=sum(below)) %>%
    filter(length_below!=0) %>%
    as.data.frame()  %>%
    select(cell_no) 
  # First month when the temperature is below 10˚C 
  dt_start_below <- filter(dt_pre_year,cell_no%in%dt_cell_cold[,1] & below==1) %>%
    group_by(cell_no) %>%
    summarise(start_below=min(month)) %>% as.data.frame() 
  # First month when the temperature is below 10˚C 
  dt_end_month_year <- filter(dt_pre_year,cell_no%in%dt_cell_cold[,1]) %>%
    inner_join(dt_start_below,by="cell_no") %>% filter((month-start_below)>0) # we keep only the cell where temperature falls below 10˚
  ...
  dt_end_month_year_during <- filter(dt_end_month_year,below==0) %>% # selection of days where temp > 10˚C
    group_by(cell_no) %>% summarise(end_month=min(month))  # selection the first month where temp > 10˚C
  # if temperature remain below 10˚C
  dt_end_month_year_above <- anti_join(dt_end_month_year, dt_end_month_year_during,by="cell_no") %>%
    filter(below==1)  
  #
  if (nrow(dt_end_month_year_above)>0) { # selection the first month where temp > 10˚C
    dt_end_month_year_above <- dt_end_month_year_above %>% 
      group_by(cell_no) %>% summarise(end_month=max(month))
  } 
...
dt_persistence_index_soe<-inner_join(mean_end_month,dt_end_month,by="cell_no") %>%
...
write.csv(file="cold_pool_indices_1959_2022.csv",x=dt_cp)
